<x-app-layout>
                    ...
                    <form method="POST" action="{{ route('profile.update') }}" class="space-y-8" enctype="multipart/form-data">
                        ...
                        <div class="mt-6">
                            ...
                            @if($user->profile_image)
                                <div class="mb-4">
                                    <img src="{{ asset($user->profile_image) }}" alt="Current Profile" class="w-32 h-32 rounded-full object-cover border-4 border-blue-200">
                        ...
                        <div class="border-b border-gray-200 pb-8">
                            ...
                            <div id="education-container" class="space-y-4">
                                @if($user->education && count($user->education) > 0)
                                    @foreach($user->education as $index => $edu)
                                    ...
                                    @foreach($user->projects as $index => $project)
                                        <div class="project-row p-4 border-2 border-gray-200 rounded-lg bg-gray-50">
                                            ...
                                            <textarea name="project_description[]" placeholder="Description" rows="2"
                                                   class="w-full px-4 py-3 border border-gray-300 rounded-lg mb-3 focus:ring-2 focus:ring-blue-500">{{ $project['description'] }}</textarea>
    ...
    <script>
        let skillIndex = {{ $user->skills ? count($user->skills) : 1 }};
        let eduIndex = {{ $user->education ? count($user->education) : 1 }};
        let projectIndex = {{ $user->projects ? count($user->projects) : 1 }};
